...
export async function fetchProducts(queryParams: {
  ...
  if (queryParams.category && queryParams.category !== 'all') {
    const catId = Number(queryParams.category);
    filtered = filtered.filter((p) => p.categoryId === catId);
  }

  if (queryParams.search && queryParams.search.trim()) {
    const cleanSearch = removeVietnameseTones(queryParams.search.toLowerCase());
    filtered = filtered.filter(
      (p) =>
        removeVietnameseTones(p.name.toLowerCase()).includes(cleanSearch) ||
        removeVietnameseTones(p.sku.toLowerCase()).includes(cleanSearch)
    );
  }

  if (queryParams.minPrice !== undefined && !isNaN(Number(queryParams.minPrice))) {
    filtered = filtered.filter((p) => p.salePrice >= Number(queryParams.minPrice));
  }

  if (queryParams.maxPrice !== undefined && !isNaN(Number(queryParams.maxPrice))) {
    filtered = filtered.filter((p) => p.salePrice <= Number(queryParams.maxPrice));
  }
...
export async function fetchProductByIdOrSlug(idOrSlug: string, companyId?: number): Promise<ProductItem | null> {
  ...
  const p = fallbackProducts.find(
    (item) => item.id === Number(idOrSlug) || item.slug === idOrSlug || item.sku === idOrSlug
  );
  return p || null;
}
...
export async function fetchBanners(companyId?: number): Promise<BannerItem[]> {
  ...
  return [
    {
      id: 1,
      title: 'Báo Giá Vật Tư Xây Dựng Mới Nhất Quý 3/2026',
      image_url: 'https://images.unsplash.com/photo-1541888946425-d0fbb186a5b3?w=1200',
      sort_order: 1,
    },
  ];
}
...
export async function createProduct(payload: ProductPayload): Promise<ProductItem> {
  ...
  const product = productFromPayload(Math.max(0, ...fallbackProducts.map((item) => item.id)) + 1, payload);
